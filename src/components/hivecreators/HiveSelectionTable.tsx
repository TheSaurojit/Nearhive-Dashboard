...
import { ApproveButton } from "./ApproveCreator"
...
export default function HiveSelectionTable() {
  const { data: rawCreators = [] } = usePendingCreatorsQuery()
  console.log(rawCreators)

  // 1) Normalize data once and precompute a lowercase search haystack.
  ...
  <ApproveButton
    ...
    onApproved={() => {
      // If you have a refetch function from React Query:
      // refetch()
      console.log("Creator approved:", row.original.userId)
    }}
  />
),
